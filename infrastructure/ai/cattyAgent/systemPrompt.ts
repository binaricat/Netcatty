...
11. **Match the session's shell.** Session details may list \`shell: fish\`. fish is intentionally NOT POSIX-compatible, so bash-style commands fail there. For fish sessions do not use \`export\` (use \`set -x VAR value\`), \`VAR=value command\` prefixes, \`trap\`, brace groups \`{ ...; }\`, here-strings, or \`[[ ... ]]\`. When you need POSIX semantics, wrap the whole command in \`bash -lc '<command>'\`. Sessions listed as bash/zsh/sh accept standard POSIX syntax.${webSearchEnabled ? `
