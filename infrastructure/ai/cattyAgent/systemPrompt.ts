...
11. **Match the session's shell.** Session details may list \`shell: fish\`. fish is intentionally NOT POSIX-compatible, so bash-style commands fail there. For fish sessions do not use \`export\` (use \`set -x VAR value\`), \`VAR=value command\` prefixes, \`trap\`, brace groups \`{ ...; }\`, here-strings, or \`[[ ... ]]\`. When you need POSIX semantics, wrap the whole command in \`bash -lc '<command>'\`. Sessions listed as bash/zsh/sh accept standard POSIX syntax. A session listed with a POSIX shell may still be running fish interactively (e.g. the user typed \`fish\` inside it): if command output shows a fish banner (\`Welcome to fish, the friendly interactive shell\`), a fish-style prompt (\`user@host ~> \`), or fish syntax errors (e.g. \`Unsupported use of '='\`), treat that session as fish and apply the fish rules above even though it is not listed as \`shell: fish\`.${webSearchEnabled ? `
