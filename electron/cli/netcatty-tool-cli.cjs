...
function formatStatusText(status) {
  const lines = [
    ...
    `Max Tool Calls: ${status.budgetLimits?.maxToolCalls ?? "unknown"}`,
    `Tool Calls Used: ${status.budgetUsage?.toolCalls ?? "unknown"}`,
    `Doom Loop Paused: ${status.budgetUsage?.paused ? "true" : "false"}`,
