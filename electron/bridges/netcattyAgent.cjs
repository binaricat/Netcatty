  ...
  sign(_pubKey, data, options, cb) {
    ...
    (async () => {
      if (this._mode === "certificate") {
        ...
        if (!privateKey) {
          log("Missing privateKey in meta", { metaKeys: Object.keys(this._meta || {}) });
          throw new Error("Missing privateKey for certificate auth");
        }

        log("Parsing private key", { keyLength: privateKey.length, hasPassphrase: !!passphrase });
        const parsed = parseKey(privateKey, passphrase);
        if (parsed instanceof Error) {
          log("Private key parse error", { error: parsed.message });
          throw parsed;
        }
        const key = Array.isArray(parsed) ? parsed[0] : parsed;
        log("Private key parsed", { keyType: key.type });
