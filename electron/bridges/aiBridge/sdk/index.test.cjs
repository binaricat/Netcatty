...
test("registry exposes SDK backends", () => {
  assert.deepEqual(listBackends().sort(), ["antigravity", "claude", "codebuddy", "codex", "copilot", "cursor", "opencode"]);
});

test("getDriver returns a driver with runTurn", () => {
  for (const key of ["antigravity", "claude", "codebuddy", "codex", "copilot", "cursor", "opencode"]) {
